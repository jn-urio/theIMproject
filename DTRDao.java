...
public class DTRDao {
    public static List<DTRRow> findByPeriodAndEmployee(Integer periodId, Integer employeeId) throws SQLException {
        ...
        String sql = "SELECT d.dtr_id, d.employee_id, e.full_name, d.date_val, d.time_in, d.time_out, d.regular_hours, d.overtime_hours, d.status " +
    ...
    public static void insert(int employeeId, Date dateVal, Time timeIn, Time timeOut, java.math.BigDecimal regularHours, java.math.BigDecimal overtimeHours, String status) throws SQLException {
        try (Connection c = Database.getConnection();
             PreparedStatement ps = c.prepareStatement(
                 "INSERT INTO DTR (employee_id, date_val, time_in, time_out, regular_hours, overtime_hours, status) VALUES (?,?,?,?,?,?,?)")) {
            ps.setInt(1, employeeId);
            ps.setDate(2, dateVal);
            ps.setTime(3, timeIn);
    ...
    public static class DTRRow {
        ...
        public final java.math.BigDecimal regularHours, overtimeHours;
        public final String status;
        public DTRRow(int dtrId, int empId, String name, Date d, Time in, Time out, java.math.BigDecimal reg, java.math.BigDecimal ot, String st) {
            this.dtrId = dtrId; employeeId = empId; fullName = name; dateVal = d; timeIn = in; timeOut = out; regularHours = reg; overtimeHours = ot; status = st;
        }
    }
}
